...
public final class QueryClient<Key: Hashable & Sendable, Output: Sendable> {
  ...
  public init(
    config: QueryConfig = .init(), fetcher: @escaping @Sendable (Key) async throws -> Output
  ) {
    self.config = config
  ...
  public func ensureQuery(_ key: Key, force: Bool = false) async {
    ...
    let now = Date()
    let stale =
      await cache.get(key).map { now.timeIntervalSince($0.updatedAt) > config.staleTime } ?? true
    if force || stale {
      await fetch(key)
    }
  }
  ...
  public func retain(_ key: Key) async {
    await cache.retain(key)
  }
  ...
  private func fetch(_ key: Key) async {
    ...
    await cache.set(
      key,
      .init(
        data: nil, error: nil, status: .loading, updatedAt: Date(),
        observers: (await cache.get(key))?.observers ?? 0))
    let task = Task(priority: .userInitiated) { [fetcher, config] () async throws -> Output in
      ...
      for attempt in 1...max(1, config.retry.maxAttempts) {
        do { return try await fetcher(key) } catch {
          lastError = error
          if attempt == config.retry.maxAttempts { throw error }
          if let delaySeconds = config.retry.delaySeconds?(attempt) {
    ...
    do {
      ...
      await cache.set(
        key,
        .init(
          data: data, error: nil, status: .success, updatedAt: Date(),
          observers: (await cache.get(key))?.observers ?? 0))
    } catch {
      await cache.set(
        key,
        .init(
          data: nil, error: error, status: .error, updatedAt: Date(),
          observers: (await cache.get(key))?.observers ?? 0))
    }
